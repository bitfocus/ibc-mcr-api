...
import { destinationPortPathParamsSchema, notFoundSchema, errorSchema } from '../../schemas'
import { prisma } from '../../utils/prisma'

// Define the DELETE /destination-ports/:destinationPortId route
export const deleteDestinationPortRoute: RouteConfig<typeof destinationPortPathParamsSchema, undefined, undefined> = {
	method: 'delete',
	...
	handler: async (req, res, next) => {
		...
		try {
			...
			if (connectedFlowEdges.length > 0) {
				...
				console.log(
					`Deleted ${connectedFlowEdges.length} flow edges connected to destination port ${destinationPortId}`,
				)
			}
