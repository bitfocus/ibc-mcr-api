...
import { destinationPostRequestBodySchema, destinationSuccessResponseSchema, errorSchema } from '../../schemas'
import { prisma } from '../../utils/prisma'

// Define the POST /destinations route
export const createDestinationRoute: RouteConfig<undefined, undefined, typeof destinationPostRequestBodySchema> = {
	method: 'post',
	path: '/destinations',
	openapi: {
	...
	handler: async (req, res, next) => {
		...
		try {
			...
			const response = {
				...destination,
				createdAt:
					destination.createdAt instanceof Date ? destination.createdAt.toISOString() : destination.createdAt,
				updatedAt:
					destination.updatedAt instanceof Date ? destination.updatedAt.toISOString() : destination.updatedAt,
			}
