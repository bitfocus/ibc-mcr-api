// Filename: route-helpers.ts (or similar)
...
export function createRouterAndApiPaths(
	...
	for (const route of routeDefinitions) {
		...
		router[route.method](route.path, validationMiddleware, route.handler as RequestHandler)

		// Construct the OpenAPI path item for this route
		// This directly uses the structure provided in `route.openapi`
		if (!openApiPaths[route.path]) {
			openApiPaths[route.path] = {}
		}
		openApiPaths[route.path][route.method] = {
